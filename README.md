...
- Analyze `.jfr`, `.hprof`, GC logs, and thread dumps through the bundled Jifa Web flow.
...
Arthas Workbench 是一个面向本地 Arthas 工作流的 IntelliJ IDEA 插件。它可以帮助你发现 IDEA Run/Debug 和本机上的 JVM 进程，用一致的流程完成 Arthas Attach，在独立工具窗口里管理多会话的 Terminal / Log，并提供 Jifa 文件分析能力与稳定的 MCP Gateway 入口，方便 AI 工具或 IDE 助手接入。
...
- 把 Terminal、Log、Web UI、Jifa 分析和 MCP Gateway 都放进同一个工作流。
...
当前仓库通过 `jifa` submodule 和 `jifa-bridge` 组合构建复用 Jifa。首次拉取后请先初始化子模块。
...
插件当前统一通过 `Open in Jifa Web` 打开浏览器版 Jifa 分析页，会在默认浏览器中打开本地 Jifa Web，并支持：
...
![jifa-analysis.png](docs/assets/jifa-analysis.png)

![jifa-jfr-web.png](docs/assets/jifa-jfr-web.png)
...
![jifa-web.png](docs/assets/jifa-web.png)
...
- `buildPlugin` 会自动通过 `jifa` submodule 构建本地 helper server jar
- 首次构建 Jifa 相关模块时会比纯插件构建更慢
